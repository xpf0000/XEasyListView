...
extension Reflect{
    ...
    //aaaa
    class func parse(json json:JSON?,replace:Dictionary<String,String>?) -> Self
    {
        let model = self.init()
                ...
                if !type.isArray {
                    ...
                    if let res = type.isAggregate(){
                        ...
                        if res is Int.Type {
                            ...
                            arrAggregate = json![key].object as! NSArray
                        }
    ...
    class func parseAggregateArray<T>(jsonArr: JSON,basicType: ReflectType.BasicType, ins: T) -> [T]{
